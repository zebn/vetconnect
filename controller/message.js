...
class Message {
    ...
    async getAllMessages(idChat){
        return new Promise((resolve, reject) => {
            db.connection.query('select m.idMessage, m.dateMessage, m.textMessage, m.binaryMessage, m.idChat, m.idUser, u.username, u.name, r.nameRole from message m inner join user u on m.idUser = u.idUser left join  role r on r.idRole = u.idRole where idChat = ? ORDER BY m.dateMessage;', [idChat], function (error, results, fields) {
